...
def RSRS(N,M,coin_id):
    sql = '''select * from kline_1h where pair = "{}" and from_unixtime(date) >= "2017-11-01" order by date'''.format(coin_id)
    ...
    df_coin['date'] = df_coin['date'].apply(lambda x: dt.datetime(time.localtime(int(x)).tm_year, time.localtime(int(x)).tm_mon, time.localtime(int(x)).tm_mday,
                                                                  time.localtime(int(x)).tm_hour))
    regr = linear_model.LinearRegression()
    #df_new = pd.DataFrame()
    date_list = pd.date_range(df_coin.date[0], df_coin.iloc[-1]['date'],freq='H')
    if len(df_coin) < M:
...
#交易——————————————————————————————————————————————————————————————
def trade(df_new, cash=1000000, cost=0.002, date_range=['2018-12-24', (dt.date.today() - dt.timedelta(1)).strftime('%Y-%m-%d')]):
    coin_id = df_new.iloc[0]['pair']
    price = [1]
    date_list = list(
        pd.date_range(dt.datetime.strptime(date_range[0], '%Y-%m-%d'), dt.datetime.strptime(date_range[-1], '%Y-%m-%d'), freq='H'))
    ...
    for date in date_list[1:]:
        ...
        df_day_before = df_new.loc[date - dt.timedelta(hours=1)]
        ...
        if df_day['gold'] and coin_pool[coin_id][0] == 0:
            coin_pool[coin_id][1] = coin_pool[coin_id][1] + cash
            coin_pool[coin_id][0] = coin_pool[coin_id][0] + cash / df_day['close']
            cash = 0
    ...
    bitcoin = pd.read_sql(
        "select * from kline_1h  where from_unixtime(date) between '{}' and '{}' and pair = '{}' order by date".format(date_range[0], date_range[1], coin_id),
        engine)
...
#    print(store)
    ...
    n1,n2,n3,v,coin_id = 13, 31, 8, 0.8, 1360
    #print(MACD(n1, n2, n3, v, coin_id))
    # RS = RSRS(12,200,'BTC-USDT')
    # print(trade(RS))
#RSRS信号参数调优，BTC(N=14, M=110),XRP(14,130), ETH(15,110), EOS(19, 130), BNB(18, 120), ADA(12, 100),XLM(17, 100),LTC(16,280)
    ...
    #[N, M] = {'BTC-USDT':(13, 110), 'ETH-USDT':(16, 120), 'XRP-USDT':(14, 130), 'EOS-USDT':(19, 100), 'XLM-USDT': (16, 100), 'LTC-USDT': (19, 290),
                # 'ETH-BTC': (13, 110), 'XRP-BTC': (15, 240), 'EOS-BTC': (19, 110), 'XLM-BTC': (16, 220), 'LTC-BTC':(18, 260)}
    for coin_id in coin_ids[0:1]:
        temp = 0
