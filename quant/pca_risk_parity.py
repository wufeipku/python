...
# 计算每类资产对总资产组合的风险贡献
def calculate_risk_contribution(weight, cov_matrix, Eigen):
    sigma = np.sqrt(weight * cov_matrix * np.matrix(weight).T)
    # 边际风险贡献 Marginal Risk Contribution (MRC)
    MRC = Eigen.T * cov_matrix * np.matrix(weight).T / sigma
    # 风险贡献 Risk Contribution (RC)
    w = Eigen.T * np.matrix(weight).T
...
def risk_budget_objective(x_weight, parameters):
    ...
    RC_real = calculate_risk_contribution(x_weight, one_cov_matrix, Eigen)
    sum_squared_error = sum(np.square(x_weight.T - RC_target.T))[0,0]
...
def calculate_portfolio_weight(N, one_cov_matrix):
    weight0 = [1/N]*N
    a, Eigen = np.linalg.eig(one_cov_matrix)
    cons = ({'type': 'eq', 'fun': constraint1}, {'type': 'ineq', 'fun': constraint2})
    res = minimize(risk_budget_objective, weight0, args=[one_cov_matrix, N, Eigen], method='SLSQP',
